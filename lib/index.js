...
export function apply(ctx) {
  ...
  function createSessionMethods(sessionId) {
    ...
    function cellHistory(args) {
      ...
      return { cellId: cell.id, versions, current_source_sha256: sha256(cell.source || ""), current: snapshotPublic() }
    }
    ...
    async function executeOne(cell, args) {
      ...
      if (!res || !res.ok) {
        ...
        return { cell, current: snapshotFast() }
      }
      ...
      return { cell, status: cell.status, durationMs: cell.durationMs, current: snapshotFast() }
    }
  ...
  registerTool(
    ...
    "Safely replace a cell's source, record a rich version snapshot, then immediately execute that cell and return the result.",
    {
