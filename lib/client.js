    ...
    function apply(ctx) {
      ...
      function useNotebook() {
        ...
        react.useEffect(function () {
          ...
          var t = setInterval(function () { call("get", {}); }, pending ? 1000 : 2500);
        ...
        render() {
          ...
          return h("div", { className: "dnb" },
            h("div", { className: "dnb-toolbar" },
              ...
              h("button", { className: "dnb-iconbtn run", onClick: function () { window.location.reload(); } }, "刷新页面")
            ),
            h("div", { className: "dnb-errbanner" }, "Notebook 前端渲染出错：" + msg),
        ...
        return h("div", { className: "dnb" },
          ...
          showCpuHint ? h("div", { className: "dnb-lb", onClick: function () { dismissCpuHint(false); } },
            h("div", { className: "dnb-cpuhint", onClick: function (e) { e.stopPropagation(); } },
              h("h4", null, "检测到大小核（混合架构）CPU"),
              h("p", null, "DSH Notebook 的 Python 计算进程可能被系统调度到小核心（E-core / 效能核），导致比 VS Code Jupyter 慢。"),
              cpuHint.platform === "win32" ? h("ul", null,
                h("li", null, "在任务管理器里右键 python.exe → 设置优先级 → 高于标准；"),
                h("li", null, "或在任务管理器 → 详细信息 → 右键 python.exe → 设置相关性，仅勾选大核（P-core / 性能核）；"),
                h("li", null, "也可在系统电源设置里把“处理器性能提升模式”设为“已启用”并关闭节能。"),
                h("li", null, "或在 cell 里用 psutil（若已安装）查看/设置："),
                h("div", { className: "dnb-cpuhint-codebox" },
                  h("button", { className: "dnb-iconbtn dnb-copybtn", title: "复制代码", onClick: function () { copyCode(COPY_CODE); } }, "⧉ 复制"),
                  h("pre", { className: "dnb-cpuhint-code" }, COPY_CODE)
                )
              ) : cpuHint.platform === "darwin" ? h("ul", null,
                h("li", null, "打开“活动监视器”（Activity Monitor）→ 双击 python 进程 → 在“采样/信息”里查看其使用的核心；"),
                h("li", null, "macOS 的调度通常会自动把重计算优先放性能核（P-core），若仍偏慢，可先确认有没有被“低功耗模式”限制；"),
                h("li", null, "检查系统设置 → 电池/电源 → 低功耗模式是否开启，若开启请关闭。")
              ) : h("ul", null,
                h("li", null, "用系统监视器（System Monitor / htop）查看 python 进程的 CPU 占用和所在核心；"),
                h("li", null, "可执行 taskset 将进程绑定到性能核（P-core），例如：taskset -pc 0-7,16-23 <pid>（按你 CPU 的实际编号调整）；"),
                h("li", null, "也可检查 CPU 调频策略，例如 cpupower frequency-set -g performance。")
              ),
              h("div", { className: "dnb-sug-row", style: { marginTop: 8 } },
                h("button", { className: "dnb-iconbtn run", onClick: function () { dismissCpuHint(true); } }, "知道了，不再提示"),
                h("button", { className: "dnb-iconbtn", onClick: function () { dismissCpuHint(false); } }, "关闭")
              )
            )
          ) : null,
